...
void BoardGame::drawTable(RenderWindow& window) {
    ...
    if (resultGame != 0) {
        ...
        // noi dung
        if (resultGame == 1) {
            resultText.setString("Nguoi choi O thang!");
            resultText.setFillColor(Color::Blue);
        }
        else if (resultGame == 2) {
            resultText.setString("Nguoi choi X thang!");
            resultText.setFillColor(Color::Red);
        }
        else if (resultGame == 3) {
            resultText.setString("Tran dau hoa!");
            resultText.setFillColor(Color(50, 50, 50));
        }

        // Vị trí
        FloatRect textBounds = resultText.getLocalBounds();
        resultText.setOrigin(textBounds.width / 2.f, textBounds.height / 2.f);
        resultText.setPosition(window.getSize().x / 2.f, window.getSize().y * 10.f / 100);
...
void BoardGame::setChoice(RenderWindow& window) {
    ...
    result(); // kiểm tra xem có người thắng chưa   
    curPlayer = 3 - curPlayer; // đổi lượt
}

void BoardGame::setMove(RenderWindow& window) {
    if (resultGame) {
        //  ESC để thoát / Enter để chơi lại     
        if (keyBoard.Esc()) {
            reset();
            state = 0; // menu
        }
        ...
        return; // dừng xử lý di chuyển
    }

    // chx ket thuc
    if (keyBoard.Up() && curY > 0) --curY;
    if (keyBoard.Down() && curY < size - 1) ++curY;
    if (keyBoard.Left() && curX > 0) --curX;
    if (keyBoard.Right() && curX < size - 1) ++curX;
    if (keyBoard.Enter()) setChoice(window);
